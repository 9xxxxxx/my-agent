...
- **重构模型回复内容排版，打破“低端问答机器人”刻板体感**：
  ...
  - **Markdown 自定义渲染器升级**：在 ReactMarkdown 中自定义了 code、table、thead 等渲染器。行内代码使用精致的浅蓝色胶囊框包裹；多行代码块直接采用深色高档 IDE 代码面板风格（带语言指示器与一键复制功能）；表格和斑马纹做了圆角、阴影与过渡重绘，使每一次分析回复在视觉上都极具艺术设计感。
