...
- **重构模型回复内容排版，打破“低端问答机器人”刻板体感**：
  - **思考过程与工具自动手风琴**：为了防止冗长的推理和密密麻麻的工具 JSON 挤占聊天记录，引入了自动手风琴折叠。推理思考面板在思考时自动展开、结束后自动收缩；工具块（ToolBlock）同样在调用中展开、调用完毕后自动折叠。历史消息的思考/工具均默认折叠，使对话界面极度利落干净。
  - **Markdown 自定义渲染器升级**：在 ReactMarkdown 中自定义了 code、table、thead、ul、ol、li 等渲染器。
    - **行内代码**：使用精致的浅蓝色胶囊框包裹；
    - **多行代码块**：直接采用深色高档 IDE 代码面板风格（带语言指示器与一键复制功能）；
    - **斑马纹表格**：表格做了圆角、阴影与过渡重绘；
    - **Bento Card 属性卡片**：智能识别无序列表（`li`）中含有的 `**属性**：描述` 结构，自动将其转为带匹配 Emoji 图标、轻阴影的 Bento 网格属性卡片。
    - **步骤条时间轴 (Timeline Steps)**：智能识别有序列表，将其渲染为带渐变数字徽章的步骤条时间轴，极大提升了阅读生动性。
  - **后端 System Prompt 升级配合**：对三大 Agent (Orchestrator, DataAnalyst, ReportWriter, GeneralAssistant) 的 System Prompt 进行了格式契合，硬性指令模型输出时遇到历程步骤和维度对比时，必须采用特定格式（如有序列表与 `加粗前缀+冒号`），以便完美触发前端的可视化重绘。
  - **打字流速调优**：正文自适应速率升级为分档呼吸公式，积压少于 8 字时限制为每帧 1 字以保留极致优雅的“一字一字吐墨感”，消除播放过快的急促和晃眼感。
